...
def calEurCallPutPrices(T, K, S, r, sigma, t):
    if(T == t):
        putp = max(K-S, 0)
        callp = max(S-K, 0)
        return [callp, putp]
...
def main():
    # We will consider Strike Price (K), sigma(sigma), rate (r) and Final Time (T)
    # 2D Plots
    ...
    for i in range(t.shape[0]):
        for j in range(K.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K[j], S, r, sig, t[i])
    ...
    for i in range(t.shape[0]):
        for j in range(sig.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K, S, r, sig[j], t[i])
    ...
    for i in range(t.shape[0]):
        for j in range(r.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K, S, r[j], sig, t[i])
    ...
    for i in range(t.shape[0]):
        for j in range(T.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T[j], K, S, r, sig, t[i])
    ...
    # 3D Plots -
    # We will consider Strike Price (K), sigma(sigma), rate (r) and Final Time (T) 2 at a time!
    # Fix t =0.5
    ...
    for i in range(K.shape[0]):
        for j in range(sig.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K[i], S, r, sig[j], t)
    ...
    ax.plot_surface(x, y, callPricesT, cmap='viridis')
    ...
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(x, y, putPricesT, cmap='viridis')
    ...
    for i in range(K.shape[0]):
        for j in range(r.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K[i], S, r[j], sig, t)
    ...
    for i in range(K.shape[0]):
        for j in range(T.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T[j], K[i], S, r, sig, t)
    ...
    for i in range(sig.shape[0]):
        for j in range(T.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T[j], K, S, r, sig[i], t)
    ...
    for i in range(sig.shape[0]):
        for j in range(r.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T, K, S, r[j], sig[i], t)
    ...
    for i in range(T.shape[0]):
        for j in range(r.shape[0]):
            [callPrices[i][j], putPrices[i][j]] = calEurCallPutPrices(T[i], K, S, r[j], sig, t)
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(x, y, callPricesT, cmap='viridis')
    ax.set_title('Surface Plot of Call Option Prices vs T and r')
    ...
    ax = fig.add_subplot(111, projection="3d")
    ax.plot_surface(x, y, putPricesT, cmap='viridis')
    ax.set_title('Surface Plot of Put Option Prices vs T and r')
    ax.set_xlabel('Expiry Time')
    ax.set_ylabel('Rate')
    ax.set_zlabel('Price of European Put Option')
    plt.show()
