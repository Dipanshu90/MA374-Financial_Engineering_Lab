...
def compare_volatility(file, opt_type, opt_name):
  options_df = pd.read_csv(file)
  stocks_df = pd.read_csv('./210123083_Data/nsedata1.csv')
  stocks_df['Date']= pd.to_datetime(stocks_df['Date'])
  stocks_df = stocks_df.interpolate(method='backfill', axis=0).ffill().bfill()
...
def main():
  files = ['NIFTYoptiondata', 'ambujastockoptions', 'apollostockoptions', 'indigostockoptions', 'marutistockoptions', 'tcsstockoptions']
  opt_type = ['NSE Index', 'Ambuja Cements', 'Apollo Hospitals', 'InterGlobe Aviation', 'Maruti Suzuki', 'TCS']
  opt_name = ['NSE Index', 'Ambuja_Cements', 'Apollo_Hospitals', 'InterGlobe_Aviation', 'Maruti_Suzuki', 'TCS']

  for idx in range(len(files)):
    files[idx] = './210123083_Data/' + files[idx] + '.csv'
    compare_volatility(files[idx], opt_type[idx], opt_name[idx])
